...
def print_statistics(car):
    ...
    print(
        f"""
        obj detection inference time mean: {np.mean(car.obj_det_model.durations[1:])}
        obj detection inference time std: {np.std(car.obj_det_model.durations[1:])}
        obj detection inference time fps: {1000 / np.mean(car.obj_det_model.durations[1:])}
        """
    )
